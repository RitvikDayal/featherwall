...
The widgets are event-driven with one deliberate exception. The clock re-arms a boundary-aligned timer; the battery and now-playing *sources* repaint only when Windows pushes a power-setting notification or a media-session event. The **now-playing record** is the exception: it runs a 15 fps frame clock to turn, gated on playing **and** desktop-uncovered **and** display-on, and creates no timer at all when `disc.rotate` is false. The clock is rendered in a Mond-inspired style — a large light-weight time, a hairline separator, and a small dimmed date — as a GDI+ bitmap drawn onto the overlay surface at most once per second (boundary-aligned timer, re-armed each tick so it never drifts).
