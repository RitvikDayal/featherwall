...
One more practical discovery: DWM did not reliably compose a *second* per-widget window target inside the wallpaper layer, so the widgets are not windows at all — each is an **overlay visual** in the same composition tree as the wallpaper content, held by key (`clock`, `info`, `halo`, `nowplaying`) so several can coexist above one content surface. Fewer HWNDs, trivial z-order, click-through by construction.

The widgets are event-driven with one deliberate exception. The clock re-arms a boundary-aligned timer; the battery and now-playing *sources* repaint only when Windows pushes a power-setting notification or a media-session event. The **now-playing record** is the exception: it runs a 15 fps frame clock to turn, gated on playing **and** desktop-uncovered **and** display-on, and when `disc.rotate` is false its timer is constructed but never armed, so nothing ever fires. The clock is rendered in a Mond-inspired style — a large light-weight time, a hairline separator, and a small dimmed date — as a GDI+ bitmap drawn onto the overlay surface at most once per second (boundary-aligned timer, re-armed each tick so it never drifts).
