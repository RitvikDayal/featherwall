...
3. **DWM applies the monitor's DPI scale to the visual tree.** On a 150 % display, visual offsets and swapchain content are scaled ×1.5 — offsets land outside the window and content gets upscale-cropped. FeatherWall sets a single counter-transform (`scale = 96 / GetDpiForWindow(hwnd)`) on the root visual, after which the entire tree works in exact physical pixels.
...
Pure logic is unit-tested (48 tests): fit-rect math, clock anchor/format/boundary-timer math, the pause decision table, config round-trip and precedence, gallery manifest invariants (hosts, licenses, checksum shape, unique ids), virtual-screen coordinate mapping. The interop layer is verified by running gates on real hardware — topology detection via `--diag`, screenshot-verified rendering behind the icons on the raised topology, measured pause/resume transitions, and wallpaper restoration on exit.
